...
* Numeric cells may include units or strip width (`12,5 мм`, `4050/ 50мм`): the leading number is stored; the unit suffix is discarded (field unit comes from the structure/property). Dual values like `900/2200 Н/50мм` or `160(+10)/100(±10)` require technician review. ± / range may also come from a **separate Excel column**: map it to the **Погрешность** slot and choose **±** or **Диапазон**.
