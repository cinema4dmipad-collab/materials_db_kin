...
- По расписанию: контейнер `backup-cron` раз в минуту вызывает `python manage.py run_scheduled_backup`. В UI задаются время запуска и **интервал в днях** (по умолчанию 1 — ежедневно; например 7 — раз в неделю). Окно запуска — 15 минут после заданного времени (если в эту минуту миграции/БД ещё не готовы, повтор в окне всё ещё сработает). Созданные файлы сохраняются в Docker volume `backups`, смонтированный как `/backups`.
...
Суперпользователь: **Администрирование → Бэкапы → Восстановить из файла**. Нужен `.dump` в формате `pg_dump -Fc` (тот же, что выдаёт кнопка «Сделать дамп сейчас»). Операция заменяет данные PostgreSQL; SeaweedFS не затрагивается. Лимит размера задаётся `BACKUP_UPLOAD_MAX_BYTES` (по умолчанию 512 МБ).
